...
describe("compress", () => {
  it("produces a valid gzip archive", async () => {
    const content = '{"_id":"A1","key":"alpha"}\n{"_id":"A2","key":"beta"}\n';
  ...
  it("returns correct size metrics", async () => {
    ...
    for (let i = 0; i < 100; i++) {
      lines.push(
        JSON.stringify({
          _id: `R${i.toString().padStart(6, "0")}`,
          key: "alpha",
          label: `sample record number ${i}`,
          value: 1000,
        }),
      );
    }
  ...
  it("achieves reasonable compression on NDJSON data", async () => {
    // Realistic-ish docs with repetitive structure
    const lines: string[] = [];
    for (let i = 0; i < 200; i++) {
      lines.push(
        JSON.stringify({
          _id: `R${i.toString().padStart(6, "0")}`,
          key: "alpha",
          label: `sample record on a repetitive structure ${i}`,
          value: 1000,
          nested: { group: "alpha", code: "G1" },
        }),
      );
    }
    const content = lines.join("\n") + "\n";
  ...
  it("compresses the synthetic NDJSON fixture", async () => {
    // crema ships a tiny synthetic fixture (no domain dependency); the address
    // fixture this test used in flat-white is not part of the shared core.
    ...
    // Verify it's a valid gzip archive that round-trips
